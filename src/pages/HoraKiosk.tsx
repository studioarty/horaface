...
import { Camera, MapPin, CheckCircle, XCircle, AlertTriangle, UserCheck, HelpCircle } from 'lucide-react';
...
const HoraKiosk = () => {
  ...
  // Strict execution lock to prevent duplicate scans
  ...
  // 2. Load Face Models & Database Data
  useEffect(() => {
    const init = async () => {
      try {
        ...
        console.error(err);
        toast.error("Erro crítico na inicialização.");
        ...
        try {
          ...
          if (detection && detection.descriptor) {
            ...
            if (match) {
              ...
              if (guy) {
                // LOCK IMMEDIATELY to prevent double matches!
                isRunning = false;
  ...
  const handleMatchedGuy = (guy: Provider) => {
    ...
    // Refresh active records in memory to ensure correct state
  ...
  // 5. Confirm and execute check-in / check-out with full validation
  const confirmPonto = async () => {
    ...
    if (!isCheckOut) {
      ...
      if (hasLocationRestriction) {
        ...
        if (assignedLoc) {
          if (!userCoords) {
            setErrorMessage("Sua localização GPS não foi lida ainda. Permita o GPS do navegador e aguarde.");
    ...
    try {
      ...
      // Reload records to keep memory sync
      await timeStore.loadRecords();
    } catch (err) {
      console.error(err);
    ...
    <div className="min-h-[100dvh] bg-[#020617] relative flex flex-col items-center justify-center p-4 overflow-hidden font-sans select-none">
      ...
      <div className="w-full max-w-md bg-slate-950/45 backdrop-blur-2xl border border-white/5 shadow-[0_0_60px_rgba(16,185,129,0.06)] rounded-[32px] overflow-hidden flex flex-col relative z-10">
        ...
        <div className="p-6 sm:p-8 flex-1 flex flex-col justify-center min-h-[42dvh]">
          ...
          {/* STATE: CONFIRMATION QUESTION SCREEN (PREVENTS ACCIDENTAL MULTIPLES) */}
          {appState === 'confirm' && matchedProvider && (
            <div className="flex flex-col items-center gap-6 animate-in slide-in-from-bottom-4 duration-500 py-4">
              <div className="relative">
                ...
                <div className="size-20 bg-slate-900/80 rounded-3xl flex items-center justify-center border border-cyan-500/30 text-cyan-400 shadow-[0_0_20px_rgba(34,211,238,0.15)]">
        ...
        <div className="py-3 px-6 text-center bg-transparent border-t border-white/5 flex items-center justify-between">
          <p className="text-[9px] text-slate-600 font-mono tracking-wider">HORAFACE PARALLEL v1.1</p>
